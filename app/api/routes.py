...
from app.api.dashboard import build_dashboard_html
from app.api.fusion import DrowsinessFusion
from app.core.state import CameraMetrics, EEGMetrics, SharedCameraState, SharedEEGState

logger = logging.getLogger(__name__)


def _coerce_float(data: dict, key: str, default: float = 0.0) -> float:
    try:
...
def _coerce_bool(data: dict, key: str, default: bool = False) -> bool:
...
def create_router(
    ...
    if enable_ingest:
        @router.post("/ingest/camera", summary="[내부] 카메라 메트릭 수신")
        def ingest_camera(payload: dict):
            data = payload if isinstance(payload, dict) else {}
            metrics = CameraMetrics(
                ear=_coerce_float(data, "ear", 0.0),
                mar=_coerce_float(data, "mar", 0.0),
                perclos=_coerce_float(data, "perclos", 0.0),
                pitch=_coerce_float(data, "pitch", 0.0),
                status=str(data.get("status", "Waiting...")),
                threshold=_coerce_float(data, "threshold", 0.25),
                is_calibrated=_coerce_bool(data, "is_calibrated", False),
            )
            camera_state.update_metrics(metrics)
            return {"status": "ok"}

        @router.post("/ingest/eeg", summary="[내부] EEG 메트릭 수신")
        def ingest_eeg(payload: dict):
            data = payload if isinstance(payload, dict) else {}
            metrics = EEGMetrics(
                alpha_power=_coerce_float(data, "alpha_power", 0.0),
                theta_power=_coerce_float(data, "theta_power", 0.0),
                beta_power=_coerce_float(data, "beta_power", 0.0),
